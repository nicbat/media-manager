...
| `media/classes/images.json`    | Class over the three PNGs; has a `related_doc` **file** field (one valid ref, one broken) + an `album` **record** field two-way **linked** to `albums.photos`; `config.icon: image`; subscribes to the `thumb` compression preset, so its members get `web` ∪ `thumb` — the Item 15 phase-2 union |
...
| `media/settings.json`          | Media-scoped prefs: `classOrder` (Item 18) + the `compression` registry (Item 15) — a workspace-wide `web` preset (WebP q80) **and** a width-bearing `thumb` (WebP q70 · 400w) that only the `images` class subscribes to                                                                         |
...
The `thumb` subscription is deliberate: the three PNGs in `images` resolve to **two** presets while the
unclassified blobs resolve to one, so a single serve exercises the union rule, a resized (width-bearing)
derivative, and the per-class subscription UI without any setup.
